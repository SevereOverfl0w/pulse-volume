...
extern fn get_server_info_cb(
    c: ?*pulse.struct_pa_context,
    optional_server_info: ?*const pulse.struct_pa_server_info,
    userdata: ?*c_void,
) void {
    if (optional_server_info) |server_info| {
        ...
        const o = pulse.pa_context_get_sink_info_list(
            c,
            get_sink_info_callback,
            @intToPtr(*c_void, @ptrToInt(default_sink_name)),
        );
        pulse.pa_operation_unref(o);
    }
}

extern fn context_subscribe_callback(
    c: ?*pulse.struct_pa_context,
...
extern fn context_state_callback(
    c: ?*pulse.pa_context,
    userdata: ?*c_void,
) void {
    if (pulse.pa_context_get_state(c) == .PA_CONTEXT_READY) {
        ...
        const o = pulse.pa_context_subscribe(
            c,
            @intToEnum(pulse.pa_subscription_mask, @enumToInt(pulse.PA_SUBSCRIPTION_MASK_SINK) |
                ...
                @enumToInt(pulse.PA_SUBSCRIPTION_MASK_CARD)),
            null,
            null,
        );
